...
SleepTab::SleepTab(QWidget *parent) : AbstractTab(parent){
    this->MyTitle->setText("<b>Your Sleeping Routine<\b>");
    this->MyModel->setTable("Activities");

    this->MyView->setModel(MyModel);
    MyModel->setFilter("Activity = 'Sleep'");
    MyModel->select();
    MyView->reset();
    ...
    connect(AddButton, &QPushButton::clicked, this, &SleepTab::OnAddButtonClicked);
    connect(DeleteButton, &QPushButton::clicked, this, &SleepTab::OnDeleteButtonClicked);
