...
PlayTab::PlayTab(QWidget *parent) : AbstractTab(parent)
{
    this->MyTitle->setText("<b>Your Playing Routine<\b>");
    this->MyModel->setTable("Activities");

    this->MyView->setModel(MyModel);
    MyModel->setFilter("Activity = 'Play'");
    MyModel->select();
    MyView->reset();
    ...
    connect(AddButton, &QPushButton::clicked, this, &PlayTab::OnAddButtonClicked);
    connect(DeleteButton, &QPushButton::clicked, this, &PlayTab::OnDeleteButtonClicked);
